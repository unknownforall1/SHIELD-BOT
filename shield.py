...
from Config import STRING, SUDO, BIO_MESSAGE, API_ID, API_HASH, STRING2, STRING3, STRING4 ,STRING5, STRING6, STRING7, STRING8 , STRING9, STRING10, NAME
import asyncio
...
loop.run_until_complete(start_shield())       
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        shield = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)     
        if len(e.text) > 5:
            bio = str(shield[0])
            text = "Changing Bio"
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(shield) == 2:
            message = str(shield[1])
            counter = int(shield[0])
            if counter > 1000:
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        if e.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None )
        smex = await e.get_reply_message()
        shield = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
        shieldsexy = shield[1:]
        if len(yukkisexy) == 2:
            message = str(shieldsexy[1])
            counter = int(shieldsexy[0])
            sleeptime = float(shield[0])
            for _ in range(counter):
...
print(text)
print("")
print("SMEX! sheild Multi Spam Bot Started Sucessfully.")
